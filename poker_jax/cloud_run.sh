#!/usr/bin/env bash
# Token-free cloud runner, started as root by cloud-init on a rented GPU box.
# Runs tests + benchmark, then trains until HOURS expires. Everything lands in
# /home/ubuntu/neuron_poker/results (world-readable) for pickup via the Lambda
# dashboard's JupyterLab; the instance is terminated from outside via the API.
set -x
BRANCH=claude/poker-sota-research-m9bqnn
HOURS=${HOURS:-4}
cd /home/ubuntu
git clone -b "$BRANCH" https://github.com/syntex01/neuron_poker
